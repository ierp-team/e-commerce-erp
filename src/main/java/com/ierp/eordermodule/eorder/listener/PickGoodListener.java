...
@Component
@Transactional
public class PickGoodListener implements TaskListener{
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        if(eOrderPoductList != null){
            for(EOrderProduct eOrderPoduct : eOrderPoductList){                            // 对每个订单产品进行库存修改操作                
                ...
                if(good != null){
                    goodsService.goodsPicking(good.getGoodsUuid(), needReduceSumNum);
                } 
            }
            eOrder.setOrderStatus(EOrderStatus.PICKED);     
            eOrder.setPickTime(new Date());
        }
    }       
}
